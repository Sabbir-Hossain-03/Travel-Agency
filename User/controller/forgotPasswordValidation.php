...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $_SESSION['forgot_form_data'] = ['email' => $email];
    ...
    if (empty($email_error) && empty($newPassword_error) && empty($confirmPassword_error) && empty($general_error)) {
        ...
        // Check if email exists in customer (User) table
        ...
        if ($res->num_rows === 0) {
            // Also try 'signup' table for compatibility
            $check_email->close();
        ...
        if ($res->num_rows > 0) {
            ...
            $_SESSION['otp_forgot_data'] = [
                ...
                'table'        => 'customer' // user-side uses customer table
            ];
            ...
            if ($mailSent) {
                ...
                $_SESSION['otp_success'] = "Verification code sent. Please check your email.";
            }
